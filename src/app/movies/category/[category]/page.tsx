...
export default function MovieCategoryPage() {
    ...
    const getCategoryDocName = (cat: string): string => {
        if (cat === 'popular') return 'popular-movies';
        return cat; // now-playing, upcoming
    };

    const getAPIFunction = async (cat: string, pageNum: number) => {
        if (cat === 'now-playing') {
            return await getNowPlayingMoviesPaginated(pageNum);
        } else if (cat === 'upcoming') {
            return await getUpcomingMoviesPaginated(pageNum);
        } else if (cat === 'popular') {
    ...
    useEffect(() => {
        const loadInitial = async () => {
            try {
                console.log(`[Category] Loading page 1 from TMDB API...`);
                const result = await getAPIFunction(category, 1);
                const initialItems = result.results.map(item => ({ ...item, media_type: 'movie' as const }));

                console.log(`[Category] Loaded ${initialItems.length} items from TMDB`);
                setItems(initialItems);
                setPage(1);
                setIsLoading(false);
            } catch (error) {
                console.error('[Category] Error loading initial items:', error);
                setIsLoading(false);
            }
        };
        loadInitial();
    ...
    const fetchMore = useCallback(async () => {
        console.log(`[Category] fetchMore called! page=${page}, hasMore=${hasMore}`);

        if (!hasMore) {
            console.log(`[Category] Stopping - hasMore is false`);
            return;
        }

        const nextPage = page + 1;
        console.log(`[Category] Fetching page ${nextPage}...`);

        try {
            const result = await getAPIFunction(category, nextPage);
            const newItems = result.results.map(item => ({ ...item, media_type: 'movie' as const }));

            console.log(`[Category] Loaded ${newItems.length} items from page ${nextPage}`);
            setItems(prev => [...prev, ...newItems]);
            setPage(nextPage);

            if (newItems.length === 0 || nextPage >= result.total_pages) {
                console.log(`[Category] Reached end. nextPage: ${nextPage}, total_pages: ${result.total_pages}`);
                setHasMore(false);
            }
        } catch (error) {
            console.error('[Category] Error fetching more items:', error);
            setHasMore(false);
        }
        ...
        <div className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-black text-white">
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="mb-8">
                    ...
                    {/* Removed item count as requested */}
                </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                        {items.map((item) => (
                            <div
                                ...
                                onClick={() => setSelectedItem({ ...item, tmdbMediaType: 'movie' } as any)}
                            >
